...
@section('title', __('messages.labs.create_title'))
@section('content')
<div class="container pt-4 d-flex flex-column align-items-center" style="min-height:70vh;">
    <div class="w-100" style="max-width: 900px;">
        <div class="text-center mb-4">
            ...
            <h2 class="fw-bold text-primary">{{ __('messages.labs.create_heading') }}</h2>
        </div>
        <div class="bg-white shadow-sm rounded-4 p-4 border border-2">
            <form method="POST" action="{{ route('labs.store') }}">
                @csrf
                <div class="mb-3">
                    <label for="name" class="form-label">{{ __('messages.labs.name') }}</label>
                ...
                <div class="d-flex gap-2 justify-content-center mt-4">
                    <button type="submit" class="btn btn-primary px-4 fw-bold"><i class="bi bi-check-circle me-1"></i> {{ __('messages.labs.save') }}</button>
                    <a href="{{ route('labs.index') }}" class="btn btn-secondary px-4 fw-bold"><i class="bi bi-x-circle me-1"></i> {{ __('messages.labs.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
